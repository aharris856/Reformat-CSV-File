...
public class ReformatCsv {
	...
	public boolean setFileName(String name)
	{
		...
		if(!endsWith(name,".csv"))return errorMsg(2); //file name does not end with .csv -> erorr
		fileName = name;
		return true;
	}
	...
	public boolean reformat(int[] newFormat)
	{
		...
		for(int i = 0; i < newFormat.length; i++) {
			...
			if(newFormat[i] == -1)return errorMsg(3); //new format contains index that is less than 0 -> error
		}
